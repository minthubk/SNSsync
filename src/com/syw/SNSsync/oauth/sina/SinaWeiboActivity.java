...
public class SinaWeiboActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		//根据是否存在缓存的适配器，显示首页
		if (RunningData.cache.sinaHomeTimeLineViewAdapter != null) {
    ...
    private void addPageMore(){
        ...
        view.setOnClickListener(new Button.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                new Thread() {
        			public void run() {
        				statusesAPI.homeTimeline(0l, 0l, RunningData.settings.statusCount, currentPage, false,
        						WeiboAPI.FEATURE.ORIGINAL, false, new APIRequestListener(
        								Constants.sina_api.RERUEST_TYPE_HOME_TIME_LINE));
        			};
        		}.start();
            }
        });
	...
	public void send(View v) {
		...
		statusesAPI.update(content, "0", "0", new APIRequestListener(Constants.sina_api.RERUEST_TYPE_UPDATE));
	}
	...
	private void refreshTimeLine() {
		...
		new Thread() {
			public void run() {
				//修改currentPage
				currentPage = 1;
				statusesAPI.homeTimeline(0l, 0l, RunningData.settings.statusCount, 1, false,
						WeiboAPI.FEATURE.ORIGINAL, false, new APIRequestListener(
								Constants.sina_api.RERUEST_TYPE_HOME_TIME_LINE));
			};
		}.start();
	...
	private void setCacheListViewAdapter() {
		adapter = RunningData.cache.sinaHomeTimeLineViewAdapter;
	...
	private class ListViewItemOnClickListener implements OnItemClickListener {
		...
		public void onItemClick(AdapterView<?> arg0, View arg1, final int arg2,long arg3) {
			AlertDialog.Builder pickDialog = new AlertDialog.Builder(SinaWeiboActivity.this);
			pickDialog.setItems(new String[]{ITEM_LONG_PRESS_CONTENT_FORWARD,ITEM_LONG_PRESS_CONTENT_COMMENT}, new DialogInterface.OnClickListener() {
				...
				public void onClick(DialogInterface dialog, int which) {
					...
					case ITEM_LONG_PRESS_INDEX_FORWARD:
						View forwardDialogView = LayoutInflater.from(SinaWeiboActivity.this).inflate(R.layout.dialog_forward, null);
						//Dialog中的UI组件
						...
						forwardButton.setOnClickListener(new View.OnClickListener() {
							...
							public void onClick(View v) {
								...
								loadingDialog = ProgressDialog.show(SinaWeiboActivity.this,"请稍候","正在转发...",true,false);
								new Thread() {
									public void run() {
										//先把要转发的tweet拿到
						...
						//组装并显示Dialog
						AlertDialog.Builder forwardDialogBuilder = new AlertDialog.Builder(SinaWeiboActivity.this);
					...
					case ITEM_LONG_PRESS_INDEX_COMMENT:
						//作和查看详情相同的操作
						Intent intent = new Intent(SinaWeiboActivity.this, SingleTweetActivity.class);
						tweet = (Tweet)adapter.getItem(arg2);
	...
	private class APIRequestListener implements RequestListener {
		...
		public void onComplete(final String data) {
			...
			case Constants.sina_api.RERUEST_TYPE_UPDATE:
				...
				message.what = Constants.sina_api.RERUEST_TYPE_UPDATE;
				break;

			case Constants.sina_api.RERUEST_TYPE_HOME_TIME_LINE:
				message = timeLineRefreshHandler.obtainMessage();
				message.obj = SinaWeiboJsonUtil.getOriginalTweets(data);
				break;
	...
	private class TimeLineRefreshHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			// TODO Auto-generated method stub
			List<Tweet> newTweets = (List<Tweet>) msg.obj;
			if (currentPage != 1) {
				//将新tweets压入集合
			...
			adapter = new TimeLineViewAdapter(SinaWeiboActivity.this, tweets);
			msgListView.setAdapter(adapter);
			//锁定位置
			...
			//将listView缓存下来
			RunningData.cache.sinaHomeTimeLineViewAdapter = adapter;
	...
	private class NotifyPublishResultHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			...
			Toast.makeText(SinaWeiboActivity.this, "发布成功", Toast.LENGTH_SHORT).show();
			//将发送的内容添加到内容容器，并提示更新
	...
	private class ToastMsgHandler extends Handler {
		@Override
		public void handleMessage(Message msg) {
			Toast.makeText(SinaWeiboActivity.this, msg.obj.toString(), Toast.LENGTH_SHORT).show();
		}
	}
