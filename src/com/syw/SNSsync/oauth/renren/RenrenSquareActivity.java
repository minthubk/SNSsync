 ...
 * 人人关注的公共主页的Activity
 */
public class RenrenSquareActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		titleTextView.setText("关注的主页君状态");
		// 设置不自动弹出软键盘
		...
		// 根据是否存在缓存的适配器，显示首页
		if (RunningData.cache.renrenSquareTimeLineViewAdapter != null) {
	...
	private class ListViewItemOnClickListener implements OnItemClickListener {
		...
		public void onItemClick(AdapterView<?> arg0, View arg1, final int arg2,long arg3) {
			AlertDialog.Builder pickDialog = new AlertDialog.Builder(RenrenSquareActivity.this);
			pickDialog.setItems(new String[]{ITEM_LONG_PRESS_CONTENT_FORWARD,ITEM_LONG_PRESS_CONTENT_COMMENT}, new DialogInterface.OnClickListener() {
				...
				public void onClick(DialogInterface dialog, int which) {
					...
					case ITEM_LONG_PRESS_INDEX_FORWARD:
						View forwardDialogView = LayoutInflater.from(RenrenSquareActivity.this).inflate(R.layout.dialog_forward, null);
						//Dialog中的UI组件
						...
						forwardButton.setOnClickListener(new View.OnClickListener() {
							...
							public void onClick(View v) {
								...
								loadingDialog = ProgressDialog.show(RenrenSquareActivity.this,"请稍候","正在转发...",true,false);
								new Thread() {
									@Override
									public void run() {
										//先把要转发的tweet拿到
						...
						//组装并显示Dialog
						AlertDialog.Builder forwardDialogBuilder = new AlertDialog.Builder(RenrenSquareActivity.this);
					...
					case ITEM_LONG_PRESS_INDEX_COMMENT:
						//作和查看详情相同的操作
						Intent intent = new Intent(RenrenSquareActivity.this, SingleTweetActivity.class);
	...
	private void setCacheListViewAdapter() {
		adapter = RunningData.cache.renrenSquareTimeLineViewAdapter;
	...
	private class TimeLineRefreshHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			...
			if (msg.what == SUCCEED_CODE) {
				...
				adapter = new TimeLineViewAdapter(RenrenSquareActivity.this, tweets);
				msgListView.setAdapter(adapter);
				//锁定位置
				...
				//将listView缓存下来
				RunningData.cache.renrenSquareTimeLineViewAdapter = adapter;
			} else {
				Toast.makeText(RenrenSquareActivity.this, "出错了~~网络没开呢吧？", Toast.LENGTH_SHORT).show();
			}
	...
	private class NotifyPublishResultHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			loadingDialog.dismiss();
			if (msg.what == SUCCEED_CODE) {
				Toast.makeText(RenrenSquareActivity.this, "发布成功", Toast.LENGTH_SHORT).show();
				String content = null;
				switch (msg.arg1) {
				//如果是发送新鲜事的
				...
				//将发送的内容添加到内容容器，并提示更新
				tweets.add(0, new Tweet(RunningData.nicks.renrenNick, content , new SimpleDateFormat("yyyy-MM-dd kk:mm:ss").format(System.currentTimeMillis())));
				adapter.notifyDataSetChanged();
				//置顶部
				msgListView.smoothScrollToPosition(0);
			} else {
				Toast.makeText(RenrenSquareActivity.this, "发布失败", Toast.LENGTH_SHORT).show();
			}
		}
	}
