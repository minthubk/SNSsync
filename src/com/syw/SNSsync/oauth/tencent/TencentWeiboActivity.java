...
public class TencentWeiboActivity extends Activity {
	...
	// api
	private static TAPI tAPI= new TAPI(OAuthConstants.OAUTH_VERSION_2_A);
	private static StatusesAPI statusesAPI = new StatusesAPI(OAuthConstants.OAUTH_VERSION_2_A);
	...
	public static String pageTime = "0";//上次的请求返回的第一条记录的时间
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		//初始化腾讯OAuthV2对象
		oAuthV2 = RunningData.oAuthV2;
		
		//在ListView中添加"加载更多".添加"加载更多"一定要在设置Adapter之前
        addPageMore();
		
		//根据是否存在缓存的适配器，显示缓存数据
		if (RunningData.cache.tencentHomeTimeLineViewAdapter != null) {
            ...
            public void onClick(View v) {
                ...
                new Thread() {
        			...
        			public void run() {
        				...
        				try {
        					String pageFlag = currentPage==1?"0":"1";
        					jsonString = statusesAPI.homeTimeline(oAuthV2, "json", pageFlag, pageTime, RunningData.settings.statusCount+"", "0x1", "0");
	...
	public void send(View v) {
		...
		new Thread() {
			public void run() {
				...
				try {
					String jsonString = tAPI.add(oAuthV2, "json", content, "127.0.0.1");
					message.obj = jsonString;
		...
		new Thread() {
			...
			public void run() {
				...
				try {
					//修改currentPage
					currentPage = 1;
					jsonString = statusesAPI.homeTimeline(oAuthV2, "json", "0", "0", RunningData.settings.statusCount+"", "0x1", "0");
					message.obj = jsonString;
					message.what = SUCCEED_CODE;
				} catch (Exception e) {
					message.what = ERROR_CODE;
	...
	private class ListViewItemOnClickListener implements OnItemClickListener {
		...
		public void onItemClick(AdapterView<?> arg0, View arg1, final int arg2,long arg3) {
			AlertDialog.Builder pickDialog = new AlertDialog.Builder(TencentWeiboActivity.this);
			pickDialog.setItems(new String[]{ITEM_LONG_PRESS_CONTENT_FORWARD,ITEM_LONG_PRESS_CONTENT_COMMENT}, new DialogInterface.OnClickListener() {
				...
				public void onClick(DialogInterface dialog, int which) {
					...
					case ITEM_LONG_PRESS_INDEX_FORWARD:
						View forwardDialogView = LayoutInflater.from(TencentWeiboActivity.this).inflate(R.layout.dialog_forward, null);
						//Dialog中的UI组件
						...
						forwardButton.setOnClickListener(new View.OnClickListener() {
							...
							public void onClick(View v) {
								...
								loadingDialog = ProgressDialog.show(TencentWeiboActivity.this,"请稍候","正在转发...",true,false);
								new Thread() {
									@Override
									public void run() {
										Message message = notifyPublishResultHandler.obtainMessage();
										...
										//请求
										try {
											tAPI.reAdd(oAuthV2, "json", forwardContent, "127.0.0.1", tweet.getId());
											message.what = SUCCEED_CODE;
										} catch (Exception e) {
											message.what = ERROR_CODE;
										}
						...
						//组装并显示Dialog
						AlertDialog.Builder forwardDialogBuilder = new AlertDialog.Builder(TencentWeiboActivity.this);
					...
					case ITEM_LONG_PRESS_INDEX_COMMENT:
						//作和查看详情相同的操作
						Intent intent = new Intent(TencentWeiboActivity.this, SingleTweetActivity.class);
						Tweet tweet = (Tweet)adapter.getItem(arg2);
						intent.putExtra("tweet", tweet);
						//赋值来源吗
						intent.setFlags(SingleTweetActivity.FROM_CODE_TENCENT);
						startActivity(intent);
						break;
					}
				}
			});
			pickDialog.setTitle("腾讯");
			pickDialog.show();
		}
	}
	...
	private void setCacheListViewAdapter() {
		adapter = RunningData.cache.tencentHomeTimeLineViewAdapter;
	...
	/**
	 * @author songyouwei
	 * 用于接收refreshTimeLine()异步传来的消息，它在主线程中被创建，更新TimeLine
	 */
	private class TimeLineRefreshHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			...
			if (msg.what == SUCCEED_CODE) {
				...
				List<Tweet> newTweets = TencentWeiboJsonUtil.getOriginalTweets(jsonString);
				if (currentPage != 1) {
					//将新tweets压入集合
				...
				adapter = new TimeLineViewAdapter(TencentWeiboActivity.this, tweets);
				msgListView.setAdapter(adapter);
				//锁定位置
				...
				//将listView缓存下来
				RunningData.cache.tencentHomeTimeLineViewAdapter = adapter;
			} else {
				Toast.makeText(TencentWeiboActivity.this, "网络出错了~~", Toast.LENGTH_SHORT).show();
			}
	...
	/**
	 * @author songyouwei
	 * 用于接收send()异步传来的消息，它在主线程中被创建，提示用户发布消息的成功与否
	 */
	private class NotifyPublishResultHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			loadingDialog.dismiss();
			if (msg.what == SUCCEED_CODE) {
				Toast.makeText(TencentWeiboActivity.this, "发布成功", Toast.LENGTH_SHORT).show();
				String content = null;
				switch (msg.arg1) {
				//如果是发送新鲜事的
				...
				//将发送的内容添加到内容容器，并提示更新
				Tweet addedTweet = new Tweet(RunningData.nicks.tencentNick, content , new SimpleDateFormat("yyyy-MM-dd kk:mm:ss").format(System.currentTimeMillis()));
				addedTweet.setUserHeadImgUrl(RunningData.heads.tencentHead);
				tweets.add(0, addedTweet);
